<template>
  <div class="space-y-8">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <!-- Ensayos Validados por Administrador -->
        <div class="bg-white/80 dark:bg-slate-900/60 p-6 rounded-3xl border border-slate-200 dark:border-white/10 space-y-6 backdrop-blur-xl transition-colors shadow-sm">
          <div class="border-b border-slate-200 dark:border-white/5 pb-3 flex justify-between items-center">
            <h2 class="text-base font-bold text-slate-900 dark:text-white">Ensayos Validados por Administrador</h2>
            <span class="text-[10px] text-slate-500 dark:text-slate-400 font-mono">participantes.id_validador</span>
          </div>

          <div class="space-y-4">
            <div v-for="adm in datos.porAdmin" :key="adm.id_usuario" class="space-y-1.5">
              <div class="flex justify-between text-xs font-bold">
                <span class="text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
                ...
                <span class="font-mono font-bold text-emerald-500 dark:text-emerald-400">
                  {{ adm.total }} validados ({{ porcentaje(adm.total, datos.totales.validados) }}%)
                </span>
              </div>
              <div class="w-full bg-slate-100 dark:bg-slate-950 rounded-full h-3 overflow-hidden border border-slate-200 dark:border-white/5">
                <div 
                  class="h-full rounded-full bg-emerald-500 transition-all duration-500"
                  :style="{ width: `${porcentajeMax(adm.total, datos.porAdmin)}%` }"
                ></div>
              </div>
            </div>

            <div v-if="datos.porAdmin.length === 0" class="py-8 text-center text-slate-500 italic text-xs">
              No hay administradores registrados en el sistema.
            </div>
          </div>
        </div>

        <!-- Ensayos Calificados por Juez -->
        <div class="bg-white/80 dark:bg-slate-900/60 p-6 rounded-3xl border border-slate-200 dark:border-white/10 space-y-6 backdrop-blur-xl transition-colors shadow-sm">
          <div class="border-b border-slate-200 dark:border-white/5 pb-3 flex justify-between items-center">
            <h2 class="text-base font-bold text-slate-900 dark:text-white">Ensayos Calificados por Juez</h2>
            <span class="text-[10px] text-slate-500 dark:text-slate-400 font-mono">calificaciones.id_juez</span>
          </div>

          <div class="space-y-4">
            <div v-for="juez in datos.porJuez" :key="juez.id_usuario" class="space-y-1.5">
              <div class="flex justify-between text-xs font-bold">
                <span class="text-slate-700 dark:text-slate-200 flex items-center gap-1.5">
                  <span class="w-2.5 h-2.5 rounded-full bg-purple-400"></span>
                  {{ juez.nombre_completo }}
                </span>
                <span class="font-mono font-bold text-purple-500 dark:text-purple-400">
                  {{ juez.total }} calificados
                </span>
              </div>
              <div class="w-full bg-slate-100 dark:bg-slate-950 rounded-full h-3 overflow-hidden border border-slate-200 dark:border-white/5">
                <div 
                  class="h-full rounded-full bg-purple-500 transition-all duration-500"
                  :style="{ width: `${porcentajeMax(juez.total, datos.porJuez)}%` }"
                ></div>
              </div>
            </div>

            <div v-if="datos.porJuez.length === 0" class="py-8 text-center text-slate-500 italic text-xs">
...
const cargarEstadisticas = async () => {
  ...
  try {
    ...
    datos.porAdmin = res.data.porAdmin || [];
    datos.porJuez = res.data.porJuez || [];
  } catch (err) {
...
const porcentajeMax = (valor, arr) => {
  if (!arr || arr.length === 0) return 0;
  const max = Math.max(...arr.map(item => item.total), 1);
  if (max === 0) return 0;
